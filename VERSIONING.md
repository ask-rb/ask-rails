...
Patch numbers advance by exactly one per release: `0.15.10` → `0.15.11` →
`0.15.12`. Never skip or jump patch numbers (`0.15.10` → `0.15.13` is
wrong), even when several fixes ship together — they ship as a single
release with a single patch number. The same one-step rule applies to minor
and major.
...
gemchain guard ask-rails
gemchain update ask-rails <new-version> --dry-run
gemchain update ask-rails <new-version>
```
...
3. Build passes — `gem build ask-rails.gemspec` (this repo's Rakefile has
   no `bundler/gem_tasks`, so there is no `rake build`).
4. Changelog — `Unreleased` entries moved under the new `X.Y.Z` heading with
   the release date.
5. Version — bumped in `lib/ask/rails/version.rb` by gemchain, not by hand.
...
| `lib/ask/rails/version.rb` at HEAD | `X.Y.Z` |
| Git tag | `vX.Y.Z` |
...
- Bug fix only: `0.15.10` → `0.15.11`.
- Several bug fixes ship together: one release, `0.15.10` → `0.15.11` (never
  `0.15.13`).
- New backwards-compatible API: `0.15.11` → `0.16.0`.
...
- This gem bumped because a dependency released: patch only, e.g. `0.15.11` →
  `0.15.12`, via the gemchain cascade — never hand-published.
